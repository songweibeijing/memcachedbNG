...
For memcachedb private command::
...
# patched by steve for memcachedb -- begin

    def get_stats(self, subcmd=''):
    ...
    def db_checkpoint(self):
        ...
        for s in self.servers:
            if not s.connect(): continue
            s.send_cmd('db_checkpoint')
            return(s.expect("OK") == "OK")

    def rep_set_priority(self, priority):
        'set priority of a replication site'
        for s in self.servers:
            if not s.connect(): continue
            s.send_cmd('rep_set_priority %d' % priority)
            return(s.expect("OK") == "OK")
 
    def rep_set_ack_policy(self, ack_policy):
        'set ack policy of a replication site'
        for s in self.servers:
            if not s.connect(): continue
            s.send_cmd('rep_set_ack_policy %d' % ack_policy)
	    return(s.expect("OK") == "OK")

# patched by steve for memcachedb -- end
...
class _Host:
    ...
    def __init__(self, host, debugfunc=None):
        ...
        if hostData.get('proto') == 'unix':
            ...
            self.port = int(hostData.get('port', 21201))
