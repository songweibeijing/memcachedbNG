...
#define DELAYLOGHOME "/data1/memecachedb_delaylog"
...
struct bdb_settings
{
    ...
    int delete_bdblogs;  /* delete the unused bdb logs */
    char *delay_dir;
...
#define ITEM_PUT        0x0001
#define ITEM_DELETE     0x0002
#define ITEM_SYNC       0x1000
...
typedef struct _stritem
{
    ...
    int             item_status;/* to add or to delete */
    int             refer_count;/* reference count */
...
void start_purge_log_thread(void);
...
item *bdb_get(char *key, size_t nkey, void *value, size_t valuesz);
item *bdb_cget(DBC *cursorp, char *start, size_t nstart, u_int32_t flags);
int bdb_put(char *key, size_t nkey, void *value, size_t valuesz);
int bdb_delete(char *key, size_t nkey);
int bdb_exists(char *key, size_t nkey);
